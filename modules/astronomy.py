...
# from discord_ui import Components, Button, SelectMenu, SelectOption, UI
...
class Astronomy(commands.Cog):
    ...
    @commands.command(
        help="Basic test command to confirm that the module is loaded and responding.\nIt is only usable by Power Users.",
    ...
    @commands.command(
        help="Gets the NASA image of the day, pass YYYY-MM-DD for a date or nothing for today!"
    )
    async def spacepic(self, ctx, arg: Optional[str]):
        if ctx.guild.id in cns.PROD_GUILDS:
            # print('Returning due to Production Guild.')
            return
        responseuser = ctx.message.author
        ...
        if apod["media_type"] == "image":
            ...
            if "hdurl" in apod.keys():
                ...
                title = (
                    d + "_" + apod["title"].replace(" ", "_").replace(":", "_") + ".jpg"
                )
                ...
                embed = discord.Embed(
                    title="Astronomy Image for " + apod.get("date") + ": ",
                    description=apod.get("title"),
                )
    ...
    async def astdatatest(self, ctx):
        ...
        lat_indices = np.where((lat_vals >= lat_range[0]) & (lat_vals <= lat_range[1]))[
            0
        ]
        lon_indices = np.where((lon_vals >= lon_range[0]) & (lon_vals <= lon_range[1]))[
            0
        ]
        ...
        start_time_index = int(
            (event_start_time - min_time).total_seconds() / (60 * 30)
        )

        # For start time check
        start_time_days = dataset["time"][start_time_index].data[0]
        start_time_date = datetime(year=1, month=1, day=1) + timedelta(
            days=start_time_days - 2
        )
        ...
        time_column = [
            event_start_time + timedelta(minutes=30 * n) for n in range(len(time))
        ]
        ...
        karachi_cantt_data = my_data[
            (my_data["Lat"] == 24.85) & (my_data["Long"] == 67.05)
        ]
        ...
        fig = px.line(
            ...
            x="Time",
            y=var_name,
            title="August 2020 Rains in Karachi (mm / hour)",
            height=400,
            width=900,
        )
        fig.show()
