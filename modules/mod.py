...
class Moderation(commands.Cog):
    ...
    async def purge(self, ctx, amt: int, user: Optional[discord.Member], filter: Optional[str]):
        ...
        if responseuser.id == responseuser.guild.owner.id or role in responseuser.roles:
    ...
    @commands.command(help="Dev Command.", brief="Power User Command", hidden=True)
    async def pingmode(self, ctx):
        member = ctx.message.author
        if (member.guild.id != 937418586409009202):  # nottebot
            return
        else:
            # print(ctx.member)
            global pingmode

            if (member.id in cns.catgirl_triad_ids):
                if (pingmode == False):
                    pingmode = True
                    await ctx.channel.send("Ping Mode is now on! Enjoy the red pings!")
                else:
                    pingmode = False
                    await ctx.channel.send("Ping Mode is now off! Enjoy the peace and quiet!")
            else:
                pass
