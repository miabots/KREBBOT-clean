...
class Economy(commands.Cog):
    ...
    async def registerecon(self, ctx):
        if await self.bot.is_prod_guild(ctx):
            return

        responseuser = ctx.message.author
        member = responseuser
        Db.dangerset("select user_id from economy")
        ...
        if str(responseuser.id) in str(testid):
            ...
            Db.dangerset(
                f"insert into economy (user_id,balance,uname) values ({member.id},1000,'{member.name}#{member.discriminator}');commit;")
    ...
    async def addcoins(self, ctx, targeto, amt):
        ...
        if target in str(testid):
            Db.dangerset(
                f"update economy set balance = balance + {amt} where {target} = user_id;commit;")
    ...
    async def addcoins2(self, ctx, targeto, amt):
        if await self.bot.is_prod_guild(ctx):
            return
        target = targeto
        # print('targeto')
    ...
    async def removecoins(self, ctx, targeto, amt: int):
        ...
        if str(target) in str(testid):
            if balold[0][0] >= amt:
                ...
                Db.dangerset(
                    f"update economy set balance = 0 where {target} = user_id;commit;")
                toss = Db.dangerexecute()
                Db.dangerset(
                    f"select balance from economy where user_id = {target}")
                bal = Db.dangerexecute()
    ...
    async def giftcoins(self, ctx, targeto, amt):
        ...
        if str(responseuser.id) in str(testid):
            ...
            if str(target) in str(testid):
                Db.dangerset(
                    f"select balance from economy where user_id = {responseuser.id}")
                bal1 = Db.dangerexecute()
                Db.dangerset(
                    f"select balance from economy where user_id = {target}")
                bal2 = Db.dangerexecute()
                ...
                Db.dangerset(
                    f"update economy set balance = balance - {amt} where {responseuser.id} = user_id;commit;")
                toss = Db.dangerexecute()
                Db.dangerset(
                    f"update economy set balance = balance + {amt} where {target} = user_id;commit;")
                toss2 = Db.dangerexecute()
                Db.dangerset(
                    f"select balance from economy where user_id = {responseuser.id}")
                bal1 = Db.dangerexecute()
                Db.dangerset(
